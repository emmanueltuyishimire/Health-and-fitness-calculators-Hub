...
export function IdealWeightCalculatorForm() {
  ...
  const handleSelectChange = (name: keyof IdealWeightFormValues) => (value: string) => {
    form.setValue(name as 'gender' | 'unitSystem' , value);
    dispatch({ type: 'SET_USER_DATA', payload: { [name]: value } });
  };
        ...
        <Separator className="my-6" />
          <div className="space-y-2 text-center md:text-left">
            ...
            <p className="text-xs text-muted-foreground pt-2">
              This range, based on the Hamwi formula, is an estimate. Body composition (muscle vs. fat) also plays a crucial role.
            </p>
          </div>
        </>
      )}
