...
export default function BmiPage() {
  ...
  const form = useForm<BmiFormValues>({
    ...
    values: { // ensures form syncs with context changes (e.g. unit system)
      height: state.height,
      weight: state.weight,
      unitSystem: state.unitSystem,
    }
      ...
      result={
        bmiResult && (
          <div className="space-y-2">
            ...
            <p className={cn("font-semibold", categoryColorClass(bmiResult.category))}>
              Category: {bmiResult.category}
            </p>
            <p className="text-xs">
              Based on the World Health Organization's recommended body weight based on height for men and women.
            </p>
          </div>
        )
      }
    >
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Button type="submit" className="w-full sm:w-auto">Calculate BMI</Button>
        </form>
      </Form>
    </CalculatorCard>
  );
