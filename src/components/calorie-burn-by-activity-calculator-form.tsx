...
import {
  CalorieBurnByActivitySchema,
  type CalorieBurnByActivityFormValues,
...
export function CalorieBurnByActivityForm() {
  const { state, dispatch } = useCalculator();
  const [result, setResult] = useState<number | null>(null);
  const [open, setOpen] = useState(false);

  const form = useForm<CalorieBurnByActivityFormValues>({
    resolver: zodResolver(CalorieBurnByActivitySchema),
    defaultValues: {
      weight: state.weight,
      activity: '',
      duration: '30',
  ...
  function onSubmit(data: CalorieBurnByActivityFormValues) {
